...
test('A → B → C 三级联邦：权限码前缀逐级组合，租户开通逐级传导', async t => {
  ...
  const a = await build({ appCode: 'a', appName: '顶层', selfApiUrl: url('a'), snapshotDebounce: 20, invalidateDebounce: 20 });
  const b = await build({ appCode: 'b', appName: '中间层', selfApiUrl: url('b'), snapshotDebounce: 20, invalidateDebounce: 20 });
  const c = await build({ appCode: 'c', appName: '末端', selfApiUrl: url('c'), snapshotDebounce: 20, invalidateDebounce: 20 });
...
test('成环拓扑不会导致无限递归：A 托管 B 且 B 托管 A 时权限树拉取被挡下', async t => {
  const ports = { a: await getFreePort(), b: await getFreePort() };
  const url = key => `http://127.0.0.1:${ports[key]}/api/integration`;

  const a = await build({ appCode: 'a', selfApiUrl: url('a'), snapshotDebounce: 20 });
  const b = await build({ appCode: 'b', selfApiUrl: url('b'), snapshotDebounce: 20 });
