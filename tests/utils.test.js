...
test('过滤只保留属于该下游的码，其它下游与上游自身的码不外泄', () => {
...
test('TTL 缓存在拉取失败时用过期数据降级', async () => {
...
test('TTL 缓存在无缓存且拉取失败时可用 fallback 兜底', async () => {
...
test('去抖把同一租户的多次变更合并成一次，并合并域集合', async () => {
  const flushed = [];
