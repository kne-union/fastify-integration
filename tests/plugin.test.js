...
describe('plugin', () => {
  ...
  it('插件能正常装载并注册两侧路由', async () => {
    const fastify = track(await build());
  ...
  it('appCode 缺失时直接拒绝启动', async () => {
    await assert.rejects(() => build({ appCode: '' }), /必须配置 options.appCode/);
  });

  it('不能把自己注册成上下游，否则权限树递归立刻成环', async () => {
    const fastify = track(await build());
  ...
  it('快照导出按 appCode 过滤权限码并剥掉一层前缀', async () => {
    const fastify = track(await build());
  ...
  it('快照投影是幂等的，重复应用不会产生重复影子行', async () => {
    const fastify = track(await build());
  ...
  it('manifest 不带前缀，权限树前缀由上游 provider 负责', async () => {
    const fastify = track(await build());
  ...
  it('权限树 provider 把下游整棵树挂在 appCode 顶层模块下', async () => {
    const fastify = track(await build());
    const { tenant } = await seedTenant(fastify);
  ...
  it('下游拉取失败时权限树降级为跳过该下游而不是整体失败', async () => {
    const fastify = track(await build());
    const { tenant } = await seedTenant(fastify);
    const { services, models } = fastify.integration;
  ...
  it('换票缓存按 tenantUserId 隔离，不同用户不会串号', async () => {
    const fastify = track(await build());
  ...
  it('install 会向下游下发我签发的回调凭据并记下 callbackAppId', async () => {
    const upstream = track(await build({ appCode: 'host-app', appName: '上游' }));
    const downstream = track(await build({ appCode: 'guest-app', appName: '下游', prefix: '/api/integration' }));
  ...
  it('两个实例可跑通开通与回拉快照的完整链路', async () => {
    ...
    const upstream = track(await build({ appCode: 'host-app', appName: '上游', selfApiUrl: upstreamUrl }));
    const downstream = track(await build({ appCode: 'guest-app', appName: '下游', selfApiUrl: downstreamUrl, snapshotDebounce: 20 }));
